...
{
    auto highest = std::sqrt(cov.GetHighestOchiaiSus());
    ...
    for (auto iter = cov.begin(index); iter != cov.end(index); iter++) {
        ...
        if (opt == Option::assign_to_coverage)
            iter->second.ptr_ranking->sus = std::sqrt(iter->second.ptr_ranking->ochiai_sus) + highest * sus;//iter->second.ptr_ranking->ochiai_sus + highest * sus;
        else if (opt == Option::maximize_coverage)
            iter->second.ptr_ranking->sus = std::sqrt(iter->second.ptr_ranking->sus) + highest * sus;//iter->second.ptr_ranking->sus + highest * sus;
    }
}
